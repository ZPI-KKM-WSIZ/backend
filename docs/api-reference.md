...
> [!WARNING]
> This backend is under active development. Additional endpoints (federation and others) are not yet available.
...
> [!NOTE]
> The `201` status code is set explicitly in the route decorator. This is likely a bug — a `GET` endpoint conventionally
> returns `200 OK`.
